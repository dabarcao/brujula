...
//
// Uso:
//   node scripts/seed-demo-360-realtext.mjs <tokenInvitacionSupervisor>
//
...
//
// Distinto de scripts/seed-demo-company.mjs (crea la empresa de cero por
// bootstrap del admin de plataforma, con datos aleatorios) y de
// scripts/seed-company-360.mjs (varios empleados de una empresa ya
// existente, texto abierto genérico de una lista corta): este parte de
// UN supervisor+empleados ya invitados, monta un único 360 completo con
// un reparto de evaluadores fijo, y usa texto libre real parafraseado
// (no genérico) para que una interpretación de IA tenga con qué
// trabajar de verdad.
...
async function main() {
  ...
  if (!supervisorInviteToken) {
    console.error("Uso: node scripts/seed-demo-360-realtext.mjs <tokenInvitacionSupervisor>");
    process.exit(1);
  }
