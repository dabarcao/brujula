...
// Takes no request parameters -- getMyCompetencyMap() has none.

import "server-only";
import { getMyCompetencyMap } from "@/server/managers/feedbackManager";
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const result = await getMyCompetencyMap();
    ...
    console.error(
      "[feedback-requests] GET /api/feedback-requests/competency-map unexpected non-Error throw:",
      error
    );
