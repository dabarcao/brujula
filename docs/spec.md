...
- **Empleado — implementado**: gráfico de araña (radar chart, SVG hecho a mano, sin librería) con las 14 competencias agrupadas por color según su dimensión (Propósito evolutivo / Equipo autoorganizado / Plenitud), en "Mi mapa de competencias" (sección 7) — agregado de todo el feedback ya revelado, actualizado a medida que llega feedback suficiente para cada competencia (respetando el umbral de la sección 6).
- **Comparativo del ciclo 360 — rediseñado (migración 0036):** en vez de una línea conectando puntos, cada competencia pinta su propio sector en forma de gajo (como un pie chart), sombreado y coloreado según su dimensión, desde el centro hasta la media de todos los evaluadores. La autoevaluación se sigue viendo como una línea en negro sobre ese fondo. Además, el empleado puede activar o desactivar, de forma interactiva, una línea por cada categoría de evaluador (jefe/equipo/empresa/otro) que ya tenga su mínimo cumplido (sección 4.1/6) — varias pueden estar activas a la vez, superpuestas.
- **Valores numéricos exactos, no solo posición en el gráfico (implementado):** el radar por sí solo solo permite estimar la nota por dónde cae el punto. Se añaden dos cosas: una escala 1-5 discreta dibujada en el hueco entre dos ejes (para poder leer a ojo qué anillo corresponde a qué nota), y una tabla debajo del gráfico con los valores exactos de cada competencia — media de todos, autoevaluación, y una columna por cada categoría de evaluador ya revelada. La tabla siempre muestra **todas** las categorías reveladas, independientemente de qué líneas estén encendidas con los chips — esos chips solo controlan qué se dibuja en el propio gráfico, no qué aparece en la tabla.
- **Empresa — implementado**: el Supervisor ve el mismo tipo de radar (sin la parte de autoevaluación ni grupos, que no aplican a nivel agregado) pero sumando a todos los empleados de la organización ("Mapa de competencias de la empresa", sección 7). Vistas agregadas por equipo/departamento (no solo por toda la empresa junta) siguen *pendientes de implementar*.
...
- Verificar de un tirón, sobre un proyecto de Supabase realmente vacío, que las 50 migraciones aplicadas en orden reconstruyen el esquema sin fallos — hoy cada una se ha comprobado por separado dando por hecho que las anteriores ya estaban puestas, nunca las 50 seguidas de un tirón.
