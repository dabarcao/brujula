...
*Herramienta de feedback anónimo, para empresas y para personas individuales. v0.8 — se añaden las **cuentas individuales**: cualquier persona puede registrarse en `/registro` sin pertenecer a ninguna empresa, pedir feedback ágil o un 360 completo por su cuenta (sin depender de que nadie le lance un ciclo), invitando a quien quiera por email — quien responde no necesita cuenta, el token de la invitación es su única credencial. El radar del ciclo 360 se rediseña: sectores sombreados por competencia (media de todos) en vez de línea, autoevaluación en negro, y ahora también líneas seleccionables por grupo de evaluador (jefe/equipo/empresa/otro), cada una oculta hasta tener sus 3 respuestas mínimas. Nueva regla de anonimato: la autoevaluación nunca cuenta hacia el mínimo de respuestas, y en un ciclo 360 no se revela nada hasta que el propio solicitante haya completado la suya. Nuevos textos de interfaz editables sin desplegar código (`platform_texts`). v0.7 (conservado más abajo) añadió los primeros informes reales de competencias, con percentil de empresa y global. Ver notas "Implementado" / "Pendiente de implementar" en cada sección, y la sección 17 para el backlog consolidado.*
...
**Revelado condicionado a la propia autoevaluación** (implementado, migración 0049): en un ciclo 360, la autoevaluación nunca cuenta hacia el mínimo de respuestas necesarias (antes sí sumaba, lo que podía revelar con menos compañeros reales de los que parecía), y además no se revela nada del informe hasta que el propio solicitante haya completado su autoevaluación — no tiene sentido ver cómo te ven los demás sin haberte visto primero a ti mismo.
...
**Pendiente:**
- Editar (cambiar los emails, o su categoría) una solicitud ya creada por una cuenta individual — hoy solo se puede crear, no modificar después, a diferencia de una empresa (sección 4.1/4.2).
...
- **En un ciclo 360, la autoevaluación nunca cuenta hacia el umbral mínimo de respuestas** (implementado, migración 0049) — el umbral protege el anonimato de terceros, y uno mismo no es un tercero. Además, no se revela nada del informe hasta que el propio solicitante haya completado su autoevaluación.
...
- Editar una solicitud ya creada (cambiar emails o categorías) — hoy solo se puede crear, a diferencia del flujo de empresa.
...
- Verificar de un tirón, sobre un proyecto de Supabase realmente vacío, que las 49 migraciones aplicadas en orden reconstruyen el esquema sin fallos — hoy cada una se ha comprobado por separado dando por hecho que las anteriores ya estaban puestas, nunca las 49 seguidas de un tirón.
