-- Brújula — arregla contaminación de datos de prueba + blinda
-- submit_feedback_response contra respuestas de preguntas que no
-- pertenecen a la plantilla de la solicitud.
--
-- Causa: al probar el ciclo 360 por API (curl) para ir más rápido que
-- clicar 18 campos a mano, el payload de prueba se construyó consultando
-- TODAS las survey_questions de la plataforma sin filtrar por template_id,
-- así que las 5 preguntas abiertas del flujo ágil (plantilla
-- default_open_feedback) se colaron como respuestas de la solicitud del
-- ciclo 360. La función nunca validaba que question_id perteneciera a la
-- plantilla correcta — bug real, no solo de los datos de prueba.

-- Limpieza: borra las respuestas contaminadas de la solicitud de prueba y
-- reabre las invitaciones para poder volver a responder correctamente.
delete from feedback_responses
where feedback_request_id = '484b7922-c9f5-46bf-9155-91b61b7a50af';

update feedback_invitations
set used_at = null
where feedback_request_id = '484b7922-c9f5-46bf-9155-91b61b7a50af';

-- Blindaje: rechaza cualquier respuesta a una pregunta que no pertenezca
-- a la plantilla de la solicitud en cuestión.
